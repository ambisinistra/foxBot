    ...
    if list_of_idle_restricted_users:
        ...
        if REPORTING:
            for user_to_report in USERS_TO_REPORT:
                try:
                    for restr_user in list_of_idle_restricted_users:
                        restr_user_info = await bot.get_chat_member(chat_id=restr_user["chat_id"], user_id=restr_user["user_id"])
                        await bot.send_message(chat_id=user_to_report, text=str(restr_user_info))
                except:
...
async def restrict_newcomers(message):
    ...
    welcome_text = "Привет! Добро пожаловать в CHAT. Нажми на кнопку, чтобы иметь возможность отправлять сообщения."
